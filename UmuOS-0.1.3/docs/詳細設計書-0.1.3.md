...
- 0.1.3 の受入条件には `su` を含めない（0.1.4以降）。
...
結論：0.1.3 では `su` は **未成立（失敗）**。同時ログイン（ttyS0/ttyS1）とは独立の課題のため、0.1.3 の受入条件には含めない。

この節は 0.1.4 以降で設計・再検証するための予定案として残す（0.1.3 での失敗観測は「12.2」を参照）。
...
### 8.1 （推奨）UEFI ブートファイルの存在確認（観測点）

環境によっては ISO 内の UEFI ブートローダが `/efi/boot/bootx64.efi` として見える。

```bash
cd ~/umu/umu_project/UmuOS-0.1.3
xorriso -indev UmuOS-0.1.3-boot.iso -ls /efi/boot
```

期待：`bootx64.efi` が列挙されること。

NOTE: `xorriso -find` は GNU `find` と互換ではないため、`-iname` や `-print` がエラーになることがある（その場合は上の `-ls` を使う）。
...
- 0.1.3 差分：ttyS1 用に `-serial tcp:127.0.0.1:${TTYS1_PORT},server,nowait,telnet` を追加し、別端末から接続して同時ログインする
...
NOTE: 接続できても何も表示されない場合がある（接続より前に出た出力は流れてこないため）。その場合は Enter を数回押すと `(none) login:` が再表示される。
...
NOTE: こちらも同様に、接続直後は Enter を数回押して `login:` を出す。
...
注意：0.1.3 ではこの項目は **未成立（失敗）**。試行すると以下のように失敗することがある。

```text
su: must be suid to work properly
```

そのため 0.1.3 の受入条件からは除外し、0.1.4 以降で別設計として扱う。
...
- QEMU 起動オプションに `-serial tcp:127.0.0.1:${TTYS1_PORT},server,nowait,telnet` が入っているか
- ホスト側から `telnet 127.0.0.1 ${TTYS1_PORT}` で接続できるか

補足（よくある罠）：

- `telnet` で接続できるのに何も出ない / `login[...]: invalid password for 'UNKNOWN' on 'ttyS1'` のようなログが出る場合、QEMU 側が telnet プロトコルに対応していない可能性がある。
  - 対処：QEMU の ttyS1 を `...,telnet` 付きで起動する（本書の `umuOSstart.sh` はその前提）
  - 代替：QEMU を raw TCP のまま使うなら、接続側は `telnet` ではなく `nc 127.0.0.1 ${TTYS1_PORT}` を使う

### 12.2 `su: must be suid to work properly`

現状（0.1.3）：`busybox.su` の setuid 仕込みを試しても本事象が発生したため、`su` は **未成立** として扱う（受入条件外）。

0.1.4 以降で切り分け・設計する際の確認観点：
