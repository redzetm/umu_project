...
- 作業ルート：`PROJECT_DIR=~/umu/umu_project/UmuOS-0.1.3`
...
sudo mount -o loop "$PROJECT_DIR/disk/disk.img" /mnt/umuos013
...
```bash
cat > "$PROJECT_DIR/initramfs/src/init.c" <<'EOF'
#define _GNU_SOURCE
#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
...
```bash
mkdir -p "$PROJECT_DIR/initramfs/rootfs"
musl-gcc -static -O2 -Wall -Wextra \
  -o "$PROJECT_DIR/initramfs/rootfs/init" \
  "$PROJECT_DIR/initramfs/src/init.c"
chmod +x "$PROJECT_DIR/initramfs/rootfs/init"
file "$PROJECT_DIR/initramfs/rootfs/init"
```

### 6.3 initrd 作成（cpio+gzip）

```bash
cd "$PROJECT_DIR/initramfs/rootfs"
find . -print0 | cpio --null -ov --format=newc | gzip -9 > "$PROJECT_DIR/initramfs/initrd.img-6.18.1"
ls -lh "$PROJECT_DIR/initramfs/initrd.img-6.18.1"
```

ISO入力へ配置：

```bash
cp "$PROJECT_DIR/initramfs/initrd.img-6.18.1" "$PROJECT_DIR/iso_root/boot/initrd.img-6.18.1"
```
...
```bash
cat > "$PROJECT_DIR/iso_root/boot/grub/grub.cfg" <<EOF
set default=0
set timeout=0
...
```bash
cp -f /usr/share/OVMF/OVMF_VARS_4M.fd "$PROJECT_DIR/run/OVMF_VARS.fd"
```
...
qemu-system-x86_64 \
  ...
  -drive if=pflash,format=raw,file="$PROJECT_DIR/run/OVMF_VARS.fd" \
  -cdrom "$PROJECT_DIR/UmuOS-0.1.3-boot.iso" \
  -drive file="$PROJECT_DIR/disk/disk.img",if=virtio,format=raw \
  -serial stdio \
