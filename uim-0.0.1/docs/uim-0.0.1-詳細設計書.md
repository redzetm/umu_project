# uim-0.0.1-詳細設計書.md
UmuOS Interactive Minimal editor (uim) — 詳細設計書（0.0.1 / 初期版）  
Target OS: UmuOS-0.1.7-base-stable（想定）

本書は uim-0.0.1 の参照実装を規定する詳細設計書であり、実装手順・関数分割・データ構造・貼り付け可能コードを提示する。

このリポジトリ運用では **本書を唯一の正** とし、矛盾がある場合は本書を優先する。
- 仕様の要約: [uim-0.0.1-仕様書.md](uim-0.0.1-仕様書.md)
- 基本設計の要約: [uim-0.0.1-基本設計書.md](uim-0.0.1-基本設計書.md)
...
- 最小 vi ライク（移動 + `0/$` + `gg/G` + `dd/yy/p` + `:wq/:q!`）だけを実装する
...
参照実装は `uim-0.0.1/uim` に存在する。本書は「1から実装できる」ように、必要ファイルの貼り付け可能コードを併記する。
...
static void draw_status(const uim_t *u, const uim_winsz_t *ws) {
  char line[256];
  const char *m = (u->mode == UIM_MODE_INSERT) ? "INSERT" : (u->mode == UIM_MODE_COLON) ? ":" : "NORMAL";
...
static void delete_line(uim_t *u) {
  const char *s = u->buf.lines[u->cur_row].data;
  if (!s) s = "";
  free(u->yank);
  u->yank = strdup(s);
  uim_buf_delete_line(&u->buf, u->cur_row);
  if (u->cur_row >= u->buf.n && u->buf.n > 0) u->cur_row = u->buf.n - 1;
  u->cur_col_byte = 0;
  set_status(u, "deleted line");
}

static void paste_below(uim_t *u) {
  if (!u->yank) return;
  uim_buf_insert_line_after(&u->buf, u->cur_row, u->yank, strlen(u->yank));
...
static void handle_key(uim_t *u, int k) {
  ...
  switch (k) {
    ...
    case 'i': u->mode = UIM_MODE_INSERT; break;
    case 'a': enter_insert_after(u); break;
    case 'p': paste_below(u); break;
