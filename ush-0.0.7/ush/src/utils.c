...
int ush_is_space_at(const char *s, size_t i, size_t *out_len) {
  ...
  if (b0 == 0xE3 && (unsigned char)s[i + 1] == 0x80 && (unsigned char)s[i + 2] == 0x80) {
...
int ush_is_blank_line(const char *line) {
  ...
  for (size_t i = 0; line[i] != '\0';) {
    size_t sl = 0;
    if (!ush_is_space_at(line, i, &sl)) return 0;
    i += (sl == 0) ? 1 : sl;
  }
  return 1;
}
