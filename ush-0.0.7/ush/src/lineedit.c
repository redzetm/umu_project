...
static int enable_raw(void) {
  ...
  t.c_iflag &= (tcflag_t)~(IXON | ICRNL | INLCR | IGNCR | ISTRIP | BRKINT | INPCK | PARMRK);
...
static void current_token_range(const char *buf, size_t len, size_t cursor, size_t *out_start, size_t *out_end) {
  ...
  size_t start = cursor;
  while (start > 0) {
    if (buf[start - 1] == ' ' || buf[start - 1] == '\t') break;
    if (start >= 3 && (unsigned char)buf[start - 3] == 0xE3 &&
        (unsigned char)buf[start - 2] == 0x80 &&
        (unsigned char)buf[start - 1] == 0x80) {
      break;
    }
    start--;
  }
...
static int list_path_matches(
  ...
  while ((de = readdir(dp)) != NULL) {
    ...
    if (cand_len >= USH_MAX_TOKEN_LEN + 1) continue;

    if (is_dir_path(full)) {
      if (cand_len + 1 >= USH_MAX_TOKEN_LEN + 1) continue;
      memcpy(out[*io_n], cand, cand_len);
      out[*io_n][cand_len] = '/';
      out[*io_n][cand_len + 1] = '\0';
    } else {
      memcpy(out[*io_n], cand, cand_len + 1);
    }
...
static int do_tab_complete(char *buf, size_t cap, size_t *io_len, size_t *io_cursor) {
  ...
  while (fn < *io_len) {
    size_t sl = 0;
    if (!ush_is_space_at(buf, fn, &sl)) break;
    fn += sl;
  }
