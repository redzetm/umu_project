...
static int expand_word_to_list(
  ...
  for (int ri = 0; ri < rawc; ri++) {
    ...
    int er = expand_one_word(st, q, raw_parts[ri], tmp);
    if (er != 0) return er;
...
static int case_pat_matches(
  ...
  char raw_parts[USH_MAX_ARGS][USH_MAX_TOKEN_LEN + 1];
  int rawc = 0;
  if (brace_split_raw(toks[pat_tok].quote, toks[pat_tok].text, raw_parts, &rawc) != 0) {
    ush_eprintf("syntax error");
    return -1;
  }
  ...
  for (int ri = 0; ri < rawc; ri++) {
    ...
    int er = expand_one_word(st, toks[pat_tok].quote, raw_parts[ri], expanded);
    if (er != 0) return -1;

    if (toks[pat_tok].quote != QUOTE_NONE) {
      unmark_inplace(expanded);
      if (strcmp(expanded, subject) == 0) return 1;
      continue;
    }
    ...
    if (fnmatch(pattern, subject, 0) == 0) return 1;
  }
