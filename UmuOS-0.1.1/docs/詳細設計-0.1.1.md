...
```bash
make mrproper
make defconfig      # defconfigの直後に3.3の処理で確認すること
make -j"$(nproc)"
```
...
rm -f disk.img  # 前回の残骸がある場合の念のための処理
...
```bash
sudo mkdir -p /mnt/umuos011      # findmnt /mnt/umuos011してみて何も出ないのが理想
...
```
file /mnt/umuos011/bin/busybox の 典型的な期待出力：

ELF 64-bit LSB pie executable, x86-64, ... , dynamically linked, interpreter /lib64/ld-linux-x86-64.so.2, ...
これは “動的リンク版 BusyBox”。動くけど、最小rootfsだと必要ライブラリが無くて起動できない可能性がある。

ELF 64-bit LSB executable, x86-64, ... , statically linked, ...
これが “静的リンク版 BusyBox” で、今回の方針（busybox-static）だとこれが一番うれしい。
...
PW root umur でセット ※ローカル環境でテスト用として公開
pw tama umut でセット ※ローカル環境でテスト用として公開
...
# （ホスト側で busybox --install を実行すると、symlink がホスト絶対パス化して壊れることがある）
...
観測（任意：先頭だけ表示して雰囲気を掴む）

```bash
cd ~/umu/umu_project/UmuOS-0.1.1/initramfs/rootfs
python3 - <<'PY'
path = "../initrd.filelist0"
...
**つまずきポイント**：`head` だけで判断すると `/init` が無いように見えることがある。

推奨（ピンポイントで grep）：

```bash
cd ~/umu/umu_project/UmuOS-0.1.1
lsinitramfs initramfs/initrd.img-6.18.1 > initramfs/initrd.list
```
...
```bash
cd ~/umu/umu_project/UmuOS-0.1.1/initramfs
rm -f initrd.unpack.cpio initrd.unpack.list
```

```bash
cd ~/umu/umu_project/UmuOS-0.1.1/initramfs
gzip -dc initrd.img-6.18.1 > initrd.unpack.cpio
```

```bash
cd ~/umu/umu_project/UmuOS-0.1.1/initramfs
cpio -t < initrd.unpack.cpio > initrd.unpack.list
```

```bash
cd ~/umu/umu_project/UmuOS-0.1.1/initramfs
grep -E '^(init|bin/switch_root)$' initrd.unpack.list
```
...
# 観測（ttyS0）を優先したい場合：GRUBメニューもシリアルへ出す
# -nographic 運用時に特に有効
...
**つまずきポイント**：環境によって `OVMF_CODE_4M.fd` のファイル名が異なる。

```bash
ls -1 /usr/share/OVMF/OVMF_CODE* 2>/dev/null || true
ls -1 /usr/share/OVMF/OVMF_VARS* 2>/dev/null || true
```

**混線防止メモ**：シリアルログを綺麗に取りたい場合は `-serial stdio -monitor none` にすると、QEMU monitor の文字が混ざりにくい。

**つまずきポイント（実例）**：`run/OVMF_VARS_umuos_0_1_1.fd` は相対パス。
- VS Code のターミナルで作業ルート以外から実行すると `No such file or directory` になりうる。
- 起動前に `pwd` を確認し、必ず `~/umu/umu_project/UmuOS-0.1.1` 直下で実行する。
