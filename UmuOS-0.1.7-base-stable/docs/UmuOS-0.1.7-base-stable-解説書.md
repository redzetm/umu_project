---
title: UmuOS-0.1.7-base-stable 解説書
date: 2026-02-23
status: accepted
related_docs:
  - "./UmuOS-0.1.7-base-stable-機能一覧.md"
  - "./UmuOS-0.1.7-base-stable-基本設計書.md"
  - "./UmuOS-0.1.7-base-stable-詳細設計書.md"
  - "./UmuOS-0.1.7-base-stable-実装ノート.md"
---

# 0. このドキュメントの目的

UmuOS-0.1.7-base-stable を「起動して使う（観測して切り分ける）」ための前提と操作手順をまとめる。
...
また、UmuOS-0.1.7-base-stable版を利用し、自身でカスタム、研究、ユーザーランドの開発など行ってもらうためのものになります。
UmuOS-0.1.7-base-stableでは、一旦機能追加はしない方針です。機能拡張は、次期UmuOS-0.1.x-devで行います。

この版は「便利に使う」よりも、「起動・初期化・ネットワーク・ログ・認証の成立」を観測できることを優先する。

# 0. Busyboxのコマンド群で実装した経緯

UmuOS-0.1.7-base-stable では、ユーザーランド（initramfs / rootfs 側）の多くを BusyBox のコマンド（applet）で揃えています。
...
そのため UmuOS-0.1.7-base-stable では、足りない部分は無理に内側で解決せず、
...
UmuOS-0.1.7-base-stable は、起動環境（例: RockyLinux 9.7）の任意ディレクトリに次の3点が揃っていれば起動できる。
...
UmuOS-0.1.7-base-stable の標準は BusyBox ash（`/bin/sh`）であります。

- スクリプト実行・互換性寄り: `/bin/sh`

## 7.2 ush（任意で持ち込む、普段使い向け）

ush は「対話操作・軽量スクリプト」向けの別シェルで、UmuOS-0.1.7-base-stable の標準同梱は前提にしない。
...
結論：UmuOS-0.1.7-base-stable において ush 0.0.4 は「必須ではない」が、入れる意味はあるかなとおもった。
