---
title: UmuOS-0.1.7-base-stable 基本設計書
date: 2026-02-11
status: baseline
related_docs:
...
# UmuOS-0.1.7-base-stable 基本設計書

## 1. 目的

- UmuOS-0.1.7-base-stable を「Kernel + initramfs + switch_root + ext4(rootfs)」の最小構成で起動できる。
...
- パッケージ管理、ユーザー追加/削除、堅牢なセキュリティ設計（研究・検証用の範囲外）。
- 複数マシン構成、複雑なネットワーク機能（VLAN/iptables等）。

## 3. スコープ（UmuOS-0.1.7-base-stableで成立させる機能）
...
- 詳細な手順（コピペ手順書）: `UmuOS-0.1.7-base-stable-詳細設計書.md`
