...
### 次にやること
- 8.1 grub.cfg 作成（root=UUID の置換と観測点の確認）
- 9 ISO 生成（grub-mkrescue）
- 10 起動スクリプト作成（scriptでホスト側ログ保存）
...
### 8. GRUB 設定（UEFIブート）
- [ ] 8.1 grub.cfg 作成
- [ ] 観測点：`root=UUID=...` が DISK_UUID に置換されている
...
- [ ] 観測点：logs/qemu-serial_YYYYmmdd_HHMMSS.log が作られる

### 11. QEMU 起動
- [ ] 11. `./umuOSstart.sh`

### 12. 受入チェック（必須）
- [ ] 12.1 initramfs観測点（cmdline/scan/matched/mount ok/switch_root）
- [ ] 12.2 ext4側で `login:` が出る
- [ ] 12.3 root/tama ログイン
- [ ] 12.4 ② /logs/boot.log が追記される
- [ ] 12.5 永続性確認
- [ ] 12.6 ① ホスト側ログ保存確認
