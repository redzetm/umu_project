...
```bash
mkdir -p ~/umu/umu_project/UmuOS-0.1.2
...
もし 1 つでも `=y` になっていない場合は、次のように **built-in（=y）へ揃えてから再ビルド**する。

```bash
cd ~/umu/umu_project/UmuOS-0.1.2/kernel/linux-6.18.1
make menuconfig

# menuconfig で設定を修正したら再ビルド
make -j"$(nproc)"
...
注：`reboot` は権限が必要なので、`tama` で失敗する場合は `root` で実行する。
...
```bash
ls -lah ~/umu/umu_project/UmuOS-0.1.2/logs/
```

注：`./umuOSstart.sh` 実行中のターミナルは QEMU のシリアル（ゲスト）に接続されている。
ホスト側の `logs/` を見るときは、VS Code で別ターミナルを開くか、必要なら QEMU を終了してから確認する。
（`-nographic -serial stdio` の場合、QEMU終了は `Ctrl+a` → `x`）

補足：端末エミュレータの設定やキー操作によっては、制御シーケンスがそのままシリアル入力として流れ、ログに `^[[6n`（ESC`[6n`）や `;5R` のような文字列が混入して見えることがある。
これはゲストの異常ではなく「端末→シリアル」へ流れた入力の記録なので、必要なら端末側の設定（Tera Term等）を見直すか、別ターミナルでログ閲覧する。
...
sudo ls -l /mnt/umuos012/sbin/init
sudo sed -n '1,120p' /mnt/umuos012/etc/inittab
sudo ls -l /mnt/umuos012/etc/init.d/rcS
sudo umount /mnt/umuos012
```
...
- `/mnt/umuos012/etc/inittab` に `ttyS0::respawn:/sbin/getty -L 115200 ttyS0 vt100` があること
- `/mnt/umuos012/etc/init.d/rcS` が実行可能（例: `-rwxr-xr-x`）であること
- `/mnt/umuos012/sbin/init` が存在すること（BusyBoxへのリンクでOK）
...
- initramfs のログに `/dev/...` の列挙（scan）が出ているか（`/dev/vda` 等が見えるか）
- `/dev` が空っぽに見える場合は `CONFIG_DEVTMPFS=y` と `CONFIG_DEVTMPFS_MOUNT=y` を優先的に疑う

### 13.3 ext4 が怪しい（原因混入の遮断）

```bash
sudo e2fsck -f ~/umu/umu_project/UmuOS-0.1.2/disk/disk.img
```
...
```bash
cd ~/umu/umu_project/UmuOS-0.1.2
gzip -dc initramfs/initrd.img-6.18.1 | cpio -t | grep -E '^(init|bin/switch_root)$'
```
