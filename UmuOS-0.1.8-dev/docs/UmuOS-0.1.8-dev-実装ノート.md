---
title: UmuOS-0.1.8-dev 実装ノート
date: 2026-02-23
status: accepted
---


# UmuOS-0.1.8-dev 実装ノート


このノートは、[UmuOS-0.1.8-dev 詳細設計書](UmuOS-0.1.8-dev-詳細設計書.md) を **Tera Term で手動実装**した際の「差分・詰まり・判断」を記録する。
...
## 付録：initramfs の `/init`（init.c）

0.1.4 から `init.c` をコピーして実装する場合、ここが一番「何を置けばいいか分からない」ポイントになりやすいので、**全文**を載せる。

- 配置先：`UmuOS-0.1.8-dev/initramfs/src/init.c`
- 由来：`UmuOS-0.1.4-base-stable/initramfs/src/init.c`（0.1.7 でも同内容）
- 詳細設計書の 5章では、この `init.c` を `musl-gcc -static` でビルドして initramfs の `/init` にする。
...
- `UmuOS-0.1.8-dev-boot.iso`（27,539,456 bytes）
...
- `UmuOS-0.1.8-dev_start.sh`（2,563 bytes）
...
- `mount` で `/dev/vda on / type ext4` が見えるか：OK
- `/logs/boot.log` が追記されるか：OK
- `date` が `JST` 表示か：OK
- NTP 同期（`[ntp_sync] before/after`）の有無：OK
