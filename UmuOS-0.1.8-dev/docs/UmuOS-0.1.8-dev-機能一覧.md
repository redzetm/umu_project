...
# UmuOS-0.1.8-dev 機能一覧

この文書は UmuOS-0.1.8-dev の「何ができるか（できないか）」を、基本設計書/詳細設計書/実装ノートの内容に準拠して一覧化する。
