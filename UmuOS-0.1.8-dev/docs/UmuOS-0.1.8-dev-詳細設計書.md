---
title: UmuOS-0.1.8-dev 詳細設計書
date: 2026-02-15
status: accepted
---

# UmuOS-0.1.8-dev 詳細設計書

目的：UmuOS-0.1.8-dev を **最初のPKGインストールから**、途中で修正を挟まずに最後まで再現できる手順にする。
...
- 作業ルート（Ubuntu）：`/home/tama/umu_project/UmuOS-0.1.8-dev`
...
```bash
mkdir -p /home/tama/umu_project/UmuOS-0.1.8-dev
cd /home/tama/umu_project/UmuOS-0.1.8-dev
...
```bash
cat > /home/tama/umu_project/UmuOS-0.1.8-dev/tools/rcS_umuos016.sh <<'EOF'
#!/bin/sh
...
chmod +x /home/tama/umu_project/UmuOS-0.1.8-dev/tools/rcS_umuos016.sh
```
...
```bash
cat > /home/tama/umu_project/UmuOS-0.1.8-dev/tools/patch_diskimg_rcS.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x /home/tama/umu_project/UmuOS-0.1.8-dev/tools/patch_diskimg_rcS.sh
```
...
```bash
cat > /home/tama/umu_project/UmuOS-0.1.8-dev/UmuOS-0.1.8-dev_start.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail

BASE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ISO_FILE="${BASE_DIR}/UmuOS-0.1.8-dev-boot.iso"
...
say() { echo "[UmuOS-0.1.8-dev_start] $*"; }
die() { echo "[UmuOS-0.1.8-dev_start] ERROR: $*" >&2; exit 1; }
...
chmod +x /home/tama/umu_project/UmuOS-0.1.8-dev/UmuOS-0.1.8-dev_start.sh
```
...
rm -rf /home/tama/umu_project/UmuOS-0.1.8-dev/kernel/build
mkdir -p /home/tama/umu_project/UmuOS-0.1.8-dev/kernel/build

make -C /home/tama/umu_project/external/linux-6.18.1 \
	O=/home/tama/umu_project/UmuOS-0.1.8-dev/kernel/build defconfig

/home/tama/umu_project/external/linux-6.18.1/scripts/config \
	--file /home/tama/umu_project/UmuOS-0.1.8-dev/kernel/build/.config \
	-e DEVTMPFS \
	-e DEVTMPFS_MOUNT \
	-e BLK_DEV_INITRD \
	-e EXT4_FS \
...
make -C /home/tama/umu_project/external/linux-6.18.1 \
	O=/home/tama/umu_project/UmuOS-0.1.8-dev/kernel/build olddefconfig

make -C /home/tama/umu_project/external/linux-6.18.1 \
	O=/home/tama/umu_project/UmuOS-0.1.8-dev/kernel/build -j4 bzImage \
	2>&1 | tee /home/tama/umu_project/UmuOS-0.1.8-dev/logs/kernel_build_bzImage.log

mkdir -p /home/tama/umu_project/UmuOS-0.1.8-dev/iso_root/boot
cp -f /home/tama/umu_project/UmuOS-0.1.8-dev/kernel/build/arch/x86/boot/bzImage \
	/home/tama/umu_project/UmuOS-0.1.8-dev/iso_root/boot/vmlinuz-6.18.1
cp -f /home/tama/umu_project/UmuOS-0.1.8-dev/kernel/build/.config \
	/home/tama/umu_project/UmuOS-0.1.8-dev/iso_root/boot/config-6.18.1

test -f /home/tama/umu_project/UmuOS-0.1.8-dev/iso_root/boot/vmlinuz-6.18.1
```
...
rm -rf /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/busybox/work
mkdir -p /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/busybox/work
rsync -a --delete /home/tama/umu_project/external/busybox-1.36.1/ \
	/home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/busybox/work/

cd /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/busybox/work
make distclean
make defconfig
...
cp -f .config /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/busybox/config-1.36.1
make -j4 2>&1 | tee /home/tama/umu_project/UmuOS-0.1.8-dev/logs/busybox_build.log
...
rm -rf /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/rootfs
mkdir -p /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/rootfs/{bin,sbin,etc,proc,sys,dev,dev/pts,run,newroot,tmp}

cp -f /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/busybox/work/busybox \
	/home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/rootfs/bin/busybox
chmod 755 /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/rootfs/bin/busybox

sudo chroot /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/rootfs /bin/busybox --install -s /bin
sudo chroot /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/rootfs /bin/busybox --install -s /sbin

cp -f /home/tama/umu_project/UmuOS-0.1.4-base-stable/initramfs/src/init.c \
	/home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/src/init.c
musl-gcc -static -O2 -Wall -Wextra \
	-o /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/rootfs/init \
	/home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/src/init.c
chmod 755 /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/rootfs/init

cd /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs
...
mkdir -p /home/tama/umu_project/UmuOS-0.1.8-dev/iso_root/boot
cp -f initrd.img-6.18.1 /home/tama/umu_project/UmuOS-0.1.8-dev/iso_root/boot/initrd.img-6.18.1

test -f /home/tama/umu_project/UmuOS-0.1.8-dev/iso_root/boot/initrd.img-6.18.1
```

補足：`init.c` の中身を追いたい場合は、[UmuOS-0.1.8-dev 実装ノート](UmuOS-0.1.8-dev-実装ノート.md) を参照。
...
rm -f /home/tama/umu_project/UmuOS-0.1.8-dev/disk/disk.img
truncate -s 4G /home/tama/umu_project/UmuOS-0.1.8-dev/disk/disk.img
mkfs.ext4 -F -U d2c0b3c3-0b5e-4d24-8c91-09b3a4fb0c15 /home/tama/umu_project/UmuOS-0.1.8-dev/disk/disk.img

sudo mkdir -p /mnt/umuos016
sudo mount -o loop /home/tama/umu_project/UmuOS-0.1.8-dev/disk/disk.img /mnt/umuos016
...
sudo cp -f /home/tama/umu_project/UmuOS-0.1.8-dev/initramfs/busybox/work/busybox /mnt/umuos016/bin/busybox
...
sudo install -m 0755 /home/tama/umu_project/UmuOS-0.1.8-dev/tools/rcS_umuos016.sh /mnt/umuos016/etc/init.d/rcS
```
...
cat > /home/tama/umu_project/UmuOS-0.1.8-dev/iso_root/boot/grub/grub.cfg <<'EOF'
set timeout=20
set default=0
...
grub-mkrescue -o /home/tama/umu_project/UmuOS-0.1.8-dev/UmuOS-0.1.8-dev-boot.iso \
	/home/tama/umu_project/UmuOS-0.1.8-dev/iso_root

ls -l /home/tama/umu_project/UmuOS-0.1.8-dev/UmuOS-0.1.8-dev-boot.iso
```


## 9. Rocky 起動（/rootに3ファイルだけ）
...
- `UmuOS-0.1.8-dev-boot.iso`　※パーミッションは644で問題ないです。
- `disk.img`（Ubuntuで作った `disk/disk.img` を `disk.img` にして置く）　※パーミッションは644で問題ないです。
- `UmuOS-0.1.8-dev_start.sh`　※パーミッションは755で問題ないです。

### 9.0 Rocky へ 3ファイルを転送　基本うまく実装できていればFTP転送でOKです
...
nc -4 -l 12345 > UmuOS-0.1.8-dev-boot.iso
nc -4 -l 12345 > disk.img
nc -4 -l 12345 > UmuOS-0.1.8-dev_start.sh
```
...
nc -4 ${ROCKY_HOST} 12345 < UmuOS-0.1.8-dev-boot.iso
nc -4 ${ROCKY_HOST} 12345 < disk/disk.img
nc -4 ${ROCKY_HOST} 12345 < UmuOS-0.1.8-dev_start.sh
```

```bash
# Rocky（受信側）
cd /root
ls -l UmuOS-0.1.8-dev-boot.iso disk.img UmuOS-0.1.8-dev_start.sh
```
...
転送は 9章の `nc` 手順を使う（ISO/disk.img/start.sh の3本を揃える）。
