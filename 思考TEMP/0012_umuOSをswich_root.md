...
# 公開（Public）版の注意（機微情報のマスク方針）

このファイルは GitHub で Public に置く前提なので、**実機固有の値は原則マスク**する。

マスク対象（例）：
- グローバルIP / プライベートIP / 経路（GW） / DNS
- MACアドレス
- ホスト名、プロバイダ名、管理画面のURL
- `root@<host>` のプロンプト等に含まれる識別子
- 個人の作業ディレクトリ絶対パス（`/home/<user>/...` など）

書き方のルール（推奨）：
- 具体値は `<PUBLIC_IP>` `<PREFIX>` `<GW_IP>` `<DNS_IP>` `<MAC_ADDR>` `<HOSTNAME>` のように置換
- コマンドはそのまま載せてよいが、出力は「出力例（伏せ字）」にする
- 「この環境では○○できない」の断定は避け、**環境依存チェック項目**として書く
...
注意：telnet自体は平文なので、到達制御（送信元制限など）とセットで運用するのが推奨。
ただし、VPS事業者側の上流FWで「23/TCP を送信元限定」できない環境もある。
その場合、NET公開するなら「露出面を固定して観測→都度対応で版を上げる」方針（下記）をより強く意識する。
...
- したがって、インターネット公開時は **外側（上流）での制御** が理想。
	- 例：23/TCP と 21/TCP を「自宅の送信元IPだけ許可」
	- 可能なら診断用の送信元（Nessus走らせる場所）も限定

環境依存チェック（公開版では値を直接書かない）：
- VPS事業者側の上流FWで「23/TCP を送信元限定」できるか
	- できるなら推奨：自宅IP（または検証用踏み台）に限定
	- できないなら：露出面固定＋観測＋都度対応、または OS内側の到達制御を検討

補足：OS内側での到達制御をやるなら、Rocky userland ではなくても「kernelのnetfilterルール」は残せる。
ただし、initramfs/UmuOS側に `nft`/`iptables` コマンドやルール投入手段を用意する必要があるため、ここでは必須要件にはしない。
...
→ だからこそ、0.2系は「露出を最小に固定」「（可能なら）外側で到達制御」「都度対応で上げる」を運用の柱にする。
（外側制御ができない場合は、露出面を固定して観測・対応サイクルを回す前提で割り切る）
...
コピー先（このリポジトリ側）：
- `UmuOS-0.2.1-dev/docs/RockyLinux9.7参考ファイル　ディレクトリ/`
...
- `/boot/grub2/grub.cfg`
	- 上の `/etc/grub2.cfg` が指す実体（環境依存。`readlink -f /etc/grub2.cfg` で確定する）
...
補足（回答）：
- 起動方式（UEFI/BIOS）と grub.cfg の実体は、必ずコマンドで確定する（環境依存）。
- grub2-mkconfig の生成先として正しいのは基本的に **`/etc/grub2.cfg` の実体**（典型は `/boot/grub2/grub.cfg`）。
...
目的：
- 0.2系は「外側（上流）で制御」が主だが、Rocky側で何をしていたかを記録しておく。

公開版の書き方（推奨）：
- 状態は「確認コマンド」だけ残し、結果は `<ENFORCING|PERMISSIVE|DISABLED>` のように伏せる
...
## 調査メモ：VPSのコンソールは tty0 / ttyS0 のどれか？（BIOS環境）

背景：
- VPS事業者のWEBコンソールが「シリアル（ttyS0）」なのか「VGA（tty0）」なのかで、
	ブートログや `switch_root` 失敗時の観測性が大きく変わる。

典型ケース（例）：
- `/proc/cmdline` に `console=...` が無い（= ttyS0 をカーネルコンソールとして使っていない）
- `cat /sys/class/tty/console/active` が `tty0`（= WEBコンソールが tty0 側の可能性）

出力例（伏せ字：kernel version等は環境識別になり得るので省略/置換）：
...
```bash
ip -br link
lo               UNKNOWN        <MAC_ADDR> <LOOPBACK,UP,LOWER_UP>
ens3             UP             <MAC_ADDR> <BROADCAST,MULTICAST,UP,LOWER_UP>

ip a
2: ens3: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc fq_codel state UP group default qlen 1000
	link/ether <MAC_ADDR> brd <BROADCAST_MAC>
	altname enp0s3
	inet <PUBLIC_IP>/<PREFIX> brd <BROADCAST_IP> scope global noprefixroute ens3
	   valid_lft forever preferred_lft forever

ip r
default via <GW_IP> dev ens3 proto <dhcp|static> metric 100
<SUBNET_CIDR> dev ens3 proto kernel scope link src <PUBLIC_IP> metric 100
```

補足（公開版の書き方）：
- そのIPが固定か、DHCP/クラウド設定必須かは **ここがズレるとtelnet以前に詰む** ので、必ず確認してメモする（ただし公開時は伏せる）
...
- この完成形をやるなら、理想は「上流のパケットフィルタ」で **送信元を絞って 23/TCP を許可**。
	- ただし環境によっては、その設定ができない
	- できない場合は「露出面固定・観測・都度対応」の運用方針で割り切る（またはOS内側で別途到達制御を設計する）
...
- VPS事業者の管理コンソール（シリアル/レスキュー等）で復旧できる手段を事前に確保する
...
3) （推奨）上流で到達制御（例：送信元制限）
	- ただし環境によっては、VPS事業者側の上流FWで送信元制限できない場合がある
...
- VPS事業者の管理コンソール（シリアル/レスキュー/ISOブート等）でログインできることを確認
...
sudo cp "$INITRD_PATH" /boot/initrd.img-umuos017
...
（推奨）上流で到達制御（例：送信元制限）ができるなら、それを入れたうえで：

上流の送信元制限ができない環境では、telnet到達試験そのものが「危険な公開」に直結する。
公開する前提のリスクを理解した上で、必要最小限の時間・露出で実施する。
...
推奨配置：専用ディレクトリ（root権限のみが読める場所）にまとめる。
...
ssh <user>@<rocky>
sudo mv /tmp/UmuOS-0.1.7-base-stable-boot.iso "$WORKDIR"/
sudo mv /tmp/disk.img "$WORKDIR"/
sudo mv /tmp/UmuOS-0.1.7-base-stable_start.sh "$WORKDIR"/
sudo chmod +x "$WORKDIR"/UmuOS-0.1.7-base-stable_start.sh
```
...
要点：
- QEMUの user-net はゲストにNAT環境を提供する（具体値は環境/実装により異なるため公開版では伏せる）
- UmuOS は `rcS` が `/etc/umu/network.conf` を読み、static IP を打つ設計
	- よって `network.conf` を user-net のゲスト側IPへ寄せるのが手堅い
...
sudo mount -o loop "$WORKDIR"/disk.img /mnt/umuos017
...
- ゲストに static で “到達可能なIP/GW” を与えられる（VPSのネットワーク仕様次第）
